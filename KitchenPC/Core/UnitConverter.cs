namespace KitchenPC
{
    ...
    {
        private static readonly float[,] ConversionMatrix =
        {
        ...
        {
            bool result = ConversionMatrix[(int)toUnit, (int)fromUnit] != -1;
            return result;
        }

        public static float Convert(float amount, Units fromUnit, Units toUnit)
        {
            if (fromUnit == toUnit)
            {
                return amount;
        ...
        public static Amount Convert(Amount amount, Units toUnit)
        {
            ...
            result.Unit = toUnit;
            result.SizeLow = amount.SizeLow.HasValue ? (float?)Convert(amount.SizeLow.Value, amount.Unit, toUnit) : null;
            result.SizeHigh = Convert(amount.SizeHigh, amount.Unit, toUnit);
